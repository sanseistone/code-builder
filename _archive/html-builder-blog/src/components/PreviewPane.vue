...
let timer = null
function update() {
  previewDoc.value = generatePreviewDoc(props.doc)
  code.value = generateHtml(props.doc)
}
watch(
  () => props.doc,
  () => {
...
function downloadHtml() {
  const name = (props.doc.title || 'article').replace(/[\\/:*?"<>|]/g, '_') + '.html'
...
<template>
  <div class="flex h-full flex-col">
    ...
    <div v-show="tab === 'preview'" class="min-h-0 flex-1 bg-gray-100 p-4">
      <iframe :srcdoc="previewDoc" class="h-full w-full rounded-lg border border-gray-200 bg-white shadow"></iframe>
    </div>

    <div v-show="tab === 'code'" class="min-h-0 flex-1 overflow-auto bg-gray-900 p-4">
      <pre class="whitespace-pre-wrap break-all text-xs leading-relaxed text-gray-100"><code>{{ code }}</code></pre>
    </div>
  </div>
</template>
