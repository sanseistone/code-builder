...
describe('内置模板', () => {
  ...
  it('样式块为图片留出与正文之间的间距', () => {
    const style = generateHtml(fullDoc({ includeStyle: true }))
    expect(style).toContain('.article-banner {')
    expect(style).toContain('.article-image {')
    // 图片为块级且下方有间距，避免与紧跟的文字贴在一起
    expect(style).toMatch(/\.article-banner\s*\{[^}]*margin:\s*0 0 28px/)
    expect(style).toMatch(/p > img[^{]*\{[^}]*margin:\s*0 0 1\.6rem/)
  })
...
describe('新增区块：视频 / PDF 弹窗 / CTA 按钮', () => {
  ...
  it('图片段落包裹模式输出 <p><img …></p>', () => {
    // 样式块里有 .article-image 的定义，断言结构时先剔掉 <style>
    const body = html.replace(/<style>[\s\S]*?<\/style>/, '')
    expect(body).toContain('<p><img src="https://x.test/p.webp" alt="p" width="1000" height="750"></p>')
